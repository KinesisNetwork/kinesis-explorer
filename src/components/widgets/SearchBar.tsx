...
export class SearchBar extends React.Component<OperationProps> {
  ...
  // clear: React.MouseEventHandler<HTMLButtonElement> = (event) => {
  //   // this.setState(({ value: '' }))
  //   const query = event.target.value
  //   this.setState({ query: event.target.value, loading: true, message: '' }, () => {
  //      this.fetchSearch(query)
...
.then ((response) => {
  const dataKau = response._embedded.records.filter ((e) => {if (e.memo){ 
...
.then ((response) => {
const dataKag = response._embedded.records.filter ((e) => {if (e.memo){ 
...
const doRecursiveRequest =  async (searchUrl) => {
  return fetch (searchUrl).then ( async (res) => {
        ...
        if(currentResult && currentResult._embedded && currentResult._embedded.records && currentResult._embedded.records.length){
        const dataKau = currentResult._embedded.records.filter ((e) => {if (e.memo){ 
          return e.memo.toLowerCase().includes(this.state.query.toLowerCase())}})
...
const doRecursive =  async (searchLink) => {
  return fetch (searchLink).then ( async (res) => {
    // if (currentResult === undefined) {
        ...
        const dataKag = Result._embedded.records.filter ((e) => {if (e.memo){ 
          return e.memo.toLowerCase().includes(this.state.query.toLowerCase())}})
    ...
    return (
      <div className='field has-addons'>
        ...
        <button
         name='query'
         value={query}
         to={`/search/memo/${this.state.value}`}
            className='button'
            onClick={this.clearInput}  
          >
             Search
